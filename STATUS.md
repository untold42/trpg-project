...
- **进一步收口（2026-09-16）**：`场景映射.md` 重写，84 个地点类型**每个只给 2~4 个场景**
  （原来 23 个类型未映射 → 曾放开全部 49 个）；新增「`室内` / `室外`」小兜底组（按叙事判断）；
  再加**场景状态机**：只在进出门 / 移动 / 地点变化 / 强制时才换背景（`engine.last_bg_location`
  + `ui_sim.scene_switch_signal`，只看旁白），防来回抖。
...
- **城门限时（新）**：`营业时间.json` 把 **城门** 定为 `卯-申`（卯时开、申时末闭）；
  `export_walkable.py` 把城门 hours 导出，前端 `walkable.ts` 按当前时辰（`setShichen`）
  判断城门通道开闭——**闭门后走不了城门**（城墙依旧阻挡）；`query_place/query_nearby`
  结果带 `营业时间`+`现在`，NPC 可据此得知；`总览.md` 规则 6 加「城门定时」。
