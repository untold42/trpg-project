...
```
数据/
├── 基础设施分配.json              ★ 差异化城市：通用设施 + 地域设施（build_world 读）
├── 源pbf/
│   ├── 扬州.pbf                  原始 OSM 二进制
...
**尺寸档**（`trpg-map/城市.py` 的 `CITY_SIZES`，皆**方形**）：`10 / 20 / 30 / 40 / 50` km²。
城市在 `draw_tiles/build_world.py` 的 `CITY_CONFIGS` 里写：

```python
"尺寸": "20",                    # → 边长 √20 ≈ 4.47 km 的方形
"城池中心": (119.4365, 32.394),  # 城池中心（可与投影中心不同）
"地域": ["水乡", "大河", "都会"],  # 基础设施分配表里的地域标签
```

`old_city_bbox` **自动派生**（显式写 `old_city_bbox` 仍可覆盖，兼容旧城）。
POI 数量 / 坊 / 城门数 / `_ward_class` 阈值 / 城外采样内径 全部按城池大小缩放。

> ⚠️ **沿海 / 临湖城市：`城池中心` 要放在陆地上**，别让方形大半压进水里。
> 例：岳阳古城中心西边 **27%** 是洞庭湖 → 东移 1.5km 后降到 **~5%**。

> ⚠️ 城池尺寸变了 → 旧 `<城市>_POI.json` 冻结表作废。
> 用 `TRPG_REFREEZE=1 python 生成.py --city <城市>` 忽略旧冻结表、重新生成并覆盖。

**地域**由 `基础设施分配.json` 决定：

| 节 | 含义 |
|---|---|
| `通用` | 每座城都有的设施（结构同旧 `ZONE_POOLS`：zone → {kind: 权重}） |
| `地域` | 按城市声明的标签追加，如 `沿海`→市舶司/盐场/造船场、`边关`→榷场/军营/校场 |
| `每档总量` | 全城**最终 `category=custom` POI** 目标总数（城内+城外，含锚点/城门/神庙）。基准 5 km²=1430，逐档 ×1.75：`[2500, 4400, 7700, 13500, 23600]` |
| `分布` | 各 zone 的**最终放置占比**（不是抽样次数）：`core` 城内 / `water` 临水 / `general` 城外近郊 / `edge` 城外远郊。默认 `0.60/0.12/0.22/0.06` —— 想让城里更密就提高 `core`，想减城外就降 `general`/`edge` |
| `限量` | 每种设施的全城上限。标量=固定（州衙/城隍庙/钟鼓楼/文庙/贡院/市舶司=1）；列表按 `[10,20,30,40,50]`（如 县衙 `[1,2,3,4,5]`、土地庙 `[8,12,16,20,24]`、市集 `[4,6,8,10,12]`） |

可用标签：`水乡 / 沿海 / 大湖 / 大河 / 边关 / 山矿 / 纺织 / 蔗糖 / 都会`。
实际 POI 池 = `通用` ∪ 命中 `地域`。改表即时生效（重跑 `build_world` 即可，不必改代码）。

> `每档总量` 是**最终 custom 总数**：生成器自动扣除锚点/城门/神庙的固定开销，使「随机生成 + 固定」贴合目标；
> 被限量的种类达上限后**从抽样池剔除**（不浪费次数）。
> 布点锚点（手放）**不受限量管**（例：扬州锚点里手放 书场×7 / 勾栏×6 / 戏台×5）。
