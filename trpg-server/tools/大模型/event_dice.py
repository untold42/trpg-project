...
event_dice.py
=============
日常骰子事件 + 旅途骰子事件（**内容生成型**骰子）。

⚠️ **已停用**：不在 LLM 工具集里（`registry._DISABLED`），不可被调用——
已由「地点定时事件 + 世界系统」取代（见 `TODO.md`）。
代码保留以便回滚/对照：设 `TRPG_EVENT_DICE=1` 可临时启用。

规则见 `trpg-world/废弃/日常骰子事件.md` 与 `旅途骰子事件.md`。
